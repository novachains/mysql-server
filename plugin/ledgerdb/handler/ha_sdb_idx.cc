...
int ldb_get_key_direction(ha_rkey_function find_flag) {
  switch (find_flag) {
...
int ldb_create_index(const KEY *key_info, Sdb_cl &cl, bool shard_by_part_id) {
  int rc = 0;
  ...
  for (; key_part != key_end; ++key_part) {
    ...
    // TODO: ASC or DESC
    key_obj_builder.append(ldb_field_name(key_part->field), 1);
  }
  if (is_unique && shard_by_part_id) {
    key_obj_builder.append(LDB_FIELD_PART_HASH_ID, 1);
  }
  key_obj = key_obj_builder.obj();

  options_builder.append(LDB_FIELD_UNIQUE, is_unique);
  options_builder.append(LDB_FIELD_NOT_NULL, all_is_not_null);
  options = options_builder.obj();

  rc = cl.create_index(key_obj, ldb_key_name(key_info), options);
  if (rc) {
    goto error;
  }
done:
...
int ldb_get_idx_order(KEY *key_info, bson::BSONObj &order, int order_direction,
  ...
  key_end = key_part + key_info->user_defined_key_parts;
  for (; key_part != key_end; ++key_part) {
    obj_builder.append(ldb_field_name(key_part->field), order_direction);
  }
  if (secondary_sort_oid) {
    obj_builder.append(LDB_OID_FIELD, 1);
  }
...
static int get_text_key_obj(const uchar *key_ptr, const KEY_PART_INFO *key_part,
  ...
  str = &org_str;
  if (!my_charset_same(org_str.charset(), &LDB_CHARSET) &&
      !my_charset_same(org_str.charset(), &my_charset_bin)) {
    rc = ldb_convert_charset(org_str, conv_str, &LDB_CHARSET);
    if (rc) {
      goto error;
    }
    str = &conv_str;
...
static void get_datetime_key_obj(const uchar *key_ptr,
  ...
  ldb_convert_charset(org_str, str_val, &LDB_CHARSET);
...
static inline int create_condition(Field *field, const KEY_PART_INFO *key_part,
  ...
  if (LDB_ERR_OK == rc) {
    if (!op_obj.isEmpty()) {
      bson::BSONObj cond = BSON(ldb_field_name(field) << op_obj);
      builder.append(cond);
    }
  }
...
int ldb_create_condition_from_key(TABLE *table, KEY *key_info,
        ...
        if (*key_ptr) {
          ...
          */
          ...
          bson::BSONObj is_null_cond =
              BSON(ldb_field_name(field) << is_null_obj);
...
my_bool ldb_is_same_index(const KEY *a, const KEY *b) {
  my_bool rs = false;
  ...
  if (strcmp(ldb_key_name(a), ldb_key_name(b)) != 0 ||
  ...
  for (uint i = 0; i < a->user_defined_key_parts; ++i) {
    field_name_a = ldb_field_name(key_part_a->field);
    field_name_b = ldb_field_name(key_part_b->field);
