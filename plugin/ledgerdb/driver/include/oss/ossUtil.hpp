...
inline UINT32 ossHash( const BYTE *v1, UINT32 s1,
...
#define ossItoa(x,y,z) if (y) { snprintf(y, z, "%d", (INT32)(x) );}
bool ossIsPowerOf2( UINT32 num, UINT32 *pSquare = nullptr ) ;
UINT64 ossNextPowerOf2( UINT32 num, UINT32 *pSquare = nullptr ) ;
...
bool operator >  (const ossTickDelta &x, const ossTickDelta &y) ;
bool operator >= (const ossTickDelta &x, const ossTickDelta &y) ;
bool operator <  (const ossTickDelta &x, const ossTickDelta &y) ;
bool operator <= (const ossTickDelta &x, const ossTickDelta &y) ;
bool operator == (const ossTickDelta &x, const ossTickDelta &y) ;

bool operator >  (const ossTick &x, const ossTick &y) ;
bool operator >= (const ossTick &x, const ossTick &y) ;
bool operator <  (const ossTick &x, const ossTick &y) ;
bool operator <= (const ossTick &x, const ossTick &y) ;
bool operator == (const ossTick &x, const ossTick &y) ;
...
class ossTickDelta : protected ossTickCore
{
   ...
   friend bool operator >  (const ossTickDelta &x, const ossTickDelta &y) ;
   friend bool operator >= (const ossTickDelta &x, const ossTickDelta &y) ;
   friend bool operator <  (const ossTickDelta &x, const ossTickDelta &y) ;
   friend bool operator <= (const ossTickDelta &x, const ossTickDelta &y) ;
   friend bool operator == (const ossTickDelta &x, const ossTickDelta &y) ;
...
{
   friend bool operator >  (const ossTick &x, const ossTick &y ) ;
   friend bool operator >= (const ossTick &x, const ossTick &y ) ;
   friend bool operator <  (const ossTick &x, const ossTick &y ) ;
   friend bool operator <= (const ossTick &x, const ossTick &y ) ;
   friend bool operator == (const ossTick &x, const ossTick &y ) ;
...
inline bool operator > (const ossTickDelta &x, const ossTickDelta &y )
{
#if defined (_WINDOWS)
...
inline bool operator > (const ossTick &x, const ossTick &y)
{
#if defined (_WINDOWS)
   return ( x.peek() > y.peek() ) ;
#elif defined (_LINUX) || defined (_AIX) || defined (_MACOS)
...
#define OSS_HEXDUMP_LINEBUFFER_SIZE  (   OSS_HEXDUMP_ADDRESS_SIZE       \
                                       ...
                                       + sizeof("\n") )
#define OSS_HEXDUMP_NULL_PREFIX    ((char *) nullptr)
...
//    '*' will be printed, e.g.
//      ...
//      0x004B : 6F66 2053 6571 756F 6961 4442 2049 6E63    of PlanetRover Inc
...
//     fop.Open( "/home/sequoiadb/sequoiadb/PlanetRover/engine/oss/ossUtil.cpp" );
//     fop.Read( sizeof( junk ), junk, &bytesRead ) ;
//     ossHexDumpBuffer( junk, sizeof(junk), myBuf, sizeof(myBuf), nullptr,
//                       OSS_HEXDUMP_PREFIX_AS_ADDR ) ;
//     printf("\n%s\n", myBuf ) ;
...
(
   ...
   const void *   szPrefix,
   UINT32         flags,
   UINT32     *   pBytesProcessed = nullptr
) ;
...
INT32 ossGetDiskInfo ( const char *pPath, INT64 &totalBytes, INT64 &freeBytes,
                       char* fsName = nullptr, INT32 fsNameSize = 0 ) ;
...
public:
   ...
   /// -1 means unlimited
   bool getLimit( const char *str,
                  int64_t &soft,
                  int64_t &hard ) const ;

   INT32 setLimit( const char *str, int64_t soft, int64_t hard ) ;
...
bool ossNetIpIsValid( const char *ip, INT32 len ) ;

bool& ossGetSignalShieldFlag() ;
...
void ossCloseAllOpenFileHandles ( UINT32 closeSTD ) ;
void ossCloseStdFds() ;

INT32 ossResetTty();
bool ossIsTimestampValid( INT64 tm );
