...
enum MSG_TYPE
{
   ...
   MSG_BS_COMMAND_REQ                  = 2022,
   MSG_BS_COMMAND_RES                  = MAKE_REPLY_TYPE(MSG_BS_COMMAND_REQ),
   //catalogue msg
...
struct MsgOpReply
{
   ...
   MsgHeader header ;     // message header
   // 28-31 bytes
   int64_t    contextID ;   // context id if client need to get more
   // 32-35 bytes
   SINT32    flags ;      // reply flags
   // 36-39 bytes
   // 1. In most cases, startFrom (>=0) is where "this" reply is starting
   // in the context given by contextID
   // 2. If flags is LDB_CLS_NOT_PRIMARY, startFrom might be the new primary
...
/// |-- header--|--{usrname:xx, passwd:xx}--|
struct MsgAuthDelUsr
{
   MsgHeader header ;
   char      data[0] ;
} ;

struct MsgOpTransBegin
{
   MsgHeader header ;
   UINT64    transID ;
   char      reserved[8] ;
...
struct MsgOpTransCommit
{
   MsgHeader header;
};

struct MsgOpTransCommitPre
{
   MsgHeader header;
   UINT32    nodeNum ;
   UINT64    nodes[0] ;
...
struct MsgOpTransRollback
{
   MsgHeader header;
};

struct MsgComSessionInitReq
{
   MsgHeader   header ;
